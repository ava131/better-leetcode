...
| `scroll.mjs` 同级还有 `server/test/diff.ts` / `prompt-order.ts` | 见下面 server 那一节 |
...
| **★45/★46/★47** | **前缀缓存**：易变块放在 system 里 → 改一行代码废掉整段历史（实测命中被钉死在 1664，未命中涨到 2629）；想省 token 改发增量，又发现**历史是从客户端"原话"重建的，上一轮的代码块整块消失**，模型只能猜代码；顺手的版本号推进还会被 `buildMessages` 的两次调用推进两遍 | ① 排布改成 `[提示词+题干][历史][易变块+问题]` ② `turn-memory` 把上一轮实际发出去的文本记下来原样复用，prompt 变 append-only，对不上就退回全文 ③ `planPrompt` 算一次、`commit()` 只在首字节后显式调 |
| **★49** | 改动顺序时把 `cr` 用在了声明之前 → TDZ 500，**单测全绿照样挂**（单测不跑 HTTP 处理器） | 每次都真的起后端跑一轮；别只跑单测 |
...
0. **重启后端**（v0.8 改了 prompt 排布，不重启不生效），然后实机确认：
   一问一答之后，`server/bl-server.log` 的 `[chat] 完成` 那行应该有
   `缓存命中=NN%`，第二轮起应该在 80% 以上。
...
**前缀缓存实测（v0.8，同一套 10 轮场景，deepseek-flash）**

| | 旧排布（v0.7） | 新排布（v0.8） |
|---|---|---|
| 第 5 轮 命中 | 54%（1664/1404） | 86%（3968/640） |
| 第 10 轮 命中 | 39%（1664/2629） | 92%（7040/631） |
| 未命中走势 | 每轮 +245，一路涨 | 稳定在 ~600 |
| 10 轮输入侧折算 | 14.6k 等效全价 token | 9.5k（**省 35%**） |

读法：旧排布的命中被**钉死在 1664**（代码块之前那段），新排布命中跟着历史一起长。
差距随轮数线性拉大（输入 O(n²) vs 输出 O(n)）。**代码越短，省的越主要来自"挪顺序"；
代码越长，省的越主要来自"发增量"。**

- 真实痛点分布：`代码` p50 只有 256 字符、p90 520、最长 853 → **现在这个体量，diff 几乎不省；
  省钱主力是挪顺序 + 少让我写长回答**
- 输出才是大头：实测思考占输出的 55%，输出单价是输入的 4~15 倍 → **输出占账单 77%~92%**
- `usage` 现在记在 `[chat] 完成` 那一行（输入/输出/缓存命中），不用再靠字符比例猜
